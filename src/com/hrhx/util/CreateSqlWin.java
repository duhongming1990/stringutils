					...
					for(int i=0;i<sqls.length;i++){
						...
						if(result.toString().equals("")){
							if("".equals(sqls[i].trim()))
								continue;
							result.append("String "+valibleName+" = \" "+sqls[i]+" \"\n");
						}
						else {
							if("".equals(sqls[i].trim()))
								continue;
							if(sqls.length-i==2){
								if (sqls[sqls.length-1].contains("--")){
									int index = sqls[sqls.length-1].indexOf("--");
									sqls[sqls.length-1] = sqls[sqls.length-1].substring(0, index);
									if("".equals(sqls[sqls.length-1].trim())){
